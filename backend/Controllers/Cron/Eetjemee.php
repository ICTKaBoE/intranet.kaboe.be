...
abstract class Eetjemee
{
    ...
    static private function ImportStudent()
    {
        ...
        foreach ($schools as $school) {
            ...
            $groups = explode(",", $school->smsGroupStudents);

            foreach ($groups as $group) {
                $students = $aaeRepo->get($group, '1');
                Log::Write(_LOGLOCATION_, _LOGTIMESTAMP_, "WARN", "Gathering AllAccountsExtended in group '{$group}'...");
                ...
                $upsert = Arrays::map($upsert, fn($u) => [
                    ...
                    'child_goout' => 0,
                    'child_free_meal' => Strings::equal($u->Cateringtarief, "Solidariteitsfonds") ? 1 : 0,
                ]);
    ...
    static private function ImportEmployee()
    {
        ...
        foreach ($schools as $school) {
            ...
            $groups = explode(",", $school->smsGroupEmployee);

            foreach ($groups as $group) {
                Log::Write(_LOGLOCATION_, _LOGTIMESTAMP_, "WARN", "Gathering AllAccountsExtended in group '{$group}'...");
                $employees = $aaeRepo->get($group, '1');
