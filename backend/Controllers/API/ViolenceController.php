...
class ViolenceController extends ApiController
{
    ...
    protected function getForm($view, $id = null)
    {
        if (Strings::equal($view, self::VIEW_SELECT)) {
            $this->appendToJson('items', (new ViolenceForm)->get());
        }
    }
    ...
    {
        if (Strings::equal($view, self::VIEW_SELECT)) {
            $this->appendToJson('items', (new Out)->get());
        }
    }
    ...
    {
        if (Strings::equal($view, self::VIEW_SELECT)) {
            $this->appendToJson('items', (new Intention)->get());
        }
    }
    ...
    {
        if (Strings::equal($view, self::VIEW_SELECT)) {
            $this->appendToJson('items', (new Cause)->get());
        }
    }
    ...
    {
        $this->getNavigationSettings("violence");
    }
    ...
    {
        $this->postNavigationSettings("violence");
    }
        ...
        foreach ($schoolIds as $index => $schoolId) {
            ...
            foreach ($items[$schoolId] as $i => $item) {
                $table["data"][$i] = [
                    ...
                    $item->formatted->factsDateTime->display,
