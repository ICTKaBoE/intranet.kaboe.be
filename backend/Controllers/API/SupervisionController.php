...
{
	// GET
	public function getFill($view, $id = null)
	{
		$user = User::getLoggedInUser();
		$events = is_null($id) ? (new SupervisionEvent)->getByUserId($user->id) : (new SupervisionEvent)->get($id)[0];
		...
		} else if ($view == "form") $this->appendToJson("fields", $events);
	...
	// POST
	public function postEvent($view, $id = null)
	{
		$schoolId = Helpers::input()->post('userMainSchoolId')?->getValue();
		...
		if ($faction !== "delete") {
			...
			if ($this->validationIsAllGood()) {
				...
				if ($isHolliday) {
					...
					Log::write(type: Log::TYPE_ERROR, description: "StartTimestamp and EndTimestamp must be out of a vacation period");
				}

				if ($this->validationIsAllGood()) {
					if (!empty($hasOverlap)) $this->setToast("Middagtoezichten - Invullen", "Je overlapt met een andere toezicht...", self::VALIDATION_STATE_INVALID);
					else if ($spansMoreThenOneDay) $this->setToast("Middagtoezichten - Invullen", "Een toezicht kan niet doorgaan in de nacht...", self::VALIDATION_STATE_INVALID);
					else {
						...
						$existingEvent->userMainSchoolId = $schoolId;
						...
						$this->setToast("Middagtoezichten - Invullen", "Middagtoezicht op " . Clock::at($existingEvent->start)->format("d/m/Y") . " van " . Clock::at($existingEvent->start)->format("H:i") . " tot en met " . Clock::at($existingEvent->end)->format("H:i") . " geregistreerd!");
						Log::write(description: "Supervision added at " . Clock::at($existingEvent->start)->format("d/m/Y") . " from " . Clock::at($existingEvent->start)->format("H:i") . " to " . Clock::at($existingEvent->end)->format("H:i"));
					}
				}
			}
		} else {
			$ids = Helpers::input()->post('ids')?->getValue();
			$existingEvent = $supervisionEventRepo->get($ids)[0];
			$existingEvent->deleted = true;
			$supervisionEventRepo->set($existingEvent);

			$this->setToast("Middagtoezichten - Invullen", "Middagtoezicht op " . Clock::at($existingEvent->start)->format("d/m/Y") . " van " . Clock::at($existingEvent->start)->format("H:i") . " tot en met " . Clock::at($existingEvent->end)->format("H:i") . " verwijderd!");
			Log::write(description: "Supervision deleted at " . Clock::at($existingEvent->start)->format("d/m/Y") . " from " . Clock::at($existingEvent->start)->format("H:i") . " to " . Clock::at($existingEvent->end)->format("H:i"));
		}
	...
	public function postSettings()
	{
		...
		foreach (DEFAULT_SETTINGS["supervision"] as $setting => $defaultValue) {
