...
window.deviceView = (info) => {
  ...
  if (category.id == window.SELECT_OTHER_ID) {
    document.getElementById("subject").parentElement.classList.remove("d-none");
    Select.GetInstance("assetId").disable();
  } else {
    document.getElementById("subject").parentElement.classList.add("d-none");

    let schoolId = Select.GetInstance("schoolId").getValue();
    let roomId = Select.GetInstance("roomId").getValue();

    if (schoolId)
      Select.GetInstance("assetId").setExtraLoadParam("schoolId", schoolId);
    if (roomId)
      Select.GetInstance("assetId").setExtraLoadParam("roomId", roomId);
    Select.GetInstance("assetId").setDetails(
      category?.linked?.category?.managementType || category?.managementType,
    );
