...
window.departmentView = () => {
	let details = Select.GetInstance("departmentId").getItemDetails();
	Select.GetInstance("classgroupId").setExtraLoadParam(
		"schoolId",
		details["linked.school.virtual"]
			? details["linked.school.linked.parentSchool.id"]
			: details.schoolId
	);
	Select.GetInstance("classgroupId").reload();
};
...
window.studentView = () => {
	Select.GetInstance("coursePerStudent").setExtraLoadParam(
		"informatStudentId",
		Select.GetInstance("informatStudentId").getValue()
	);
	Select.GetInstance("coursePerStudent").setExtraLoadParam(
		"classgroupId",
		Select.GetInstance("classgroupId").getValue()
	);
...
let btnAdd = new Button({
	...
	options: {
		onclick: () => {
			let student =
				Select.GetInstance("informatStudentId").getItemDetails();

			let course = Select.GetInstance(
				Select.GetInstance("typeId").getItemDetails()
			...
			let department =
				Select.GetInstance("departmentId").getItemDetails();
			...
			let data = {
				...
				className:
					Select.GetInstance("classgroupId").getItemDetails().name,
