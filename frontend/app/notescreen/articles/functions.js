...
window.add = () => {
    let schoolId = Select.GetInstance(`sel${pageId}`).getValue();

    Form.GetInstance(`frm${pageId}`).reset();
    Form.GetInstance(`frm${pageId}`).setActiveType("create");
    Form.GetInstance(`frm${pageId}`).setField("schoolId", schoolId);

    Select.GetInstance("notescreenPageId").setExtraLoadParam(
        "schoolId",
        schoolId
    );
    Select.GetInstance("notescreenPageId").reload();

    Helpers.toggleModal("notescreen-articles");
};
