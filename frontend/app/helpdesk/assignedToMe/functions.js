...
window.claim = () => {
    ...
    if (values.length === 0) {
        alert("Gelieve 1 of meerdere tickets te selecteren!");
        return;
    }

    let claimUrl = Table.GetInstance(pageId).source + "/claim";

    Helpers.toggleWait();
    $.post(claimUrl, { ids: values }).always((returnData) => {
        let data = JSON.parse(
            returnData.responseText || JSON.stringify(returnData)
        );

        if (data.reloadTable) $.each(Table.INSTANCES, (id, t) => t.reload());
...
Helpers.addFloatingButton(btnFilter, btnView, btnClaim, btnBulkUpdate);

$(document).ready(() => {
    Table.GetInstance(`tbl${pageId}`).attachButton(btnView, "==1");
    Table.GetInstance(`tbl${pageId}`).attachButton(btnClaim, ">0");
