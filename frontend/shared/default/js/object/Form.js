...
export default class Form extends MasterObject {
  ...
  constructor(element) {
    ...
    this.method = (this.element.getAttribute("method") || "POST").toUpperCase();
  ...
  setActiveStep = (counter) => {
    this.activeStep =
      counter <= 1 ? 1 : counter >= this.getSteps() ? this.getSteps() : counter;
  ...
  getPrevStepObject = () =>
    $("div[data-step]").filter(`div[data-step="${this.activeStep - 1}"]`)[0];
  getNextStepObject = () =>
    $("div[data-step]").filter(`div[data-step="${this.activeStep + 1}"]`)[0];
  ...
  disable = () => {
    $(this.element)
      ...
      .each((idx, el) => {
        ...
        if (el.tagName === "SELECT") Select.GetInstance(el.id)?.disable();
  ...
  enable = () => {
    $(this.element)
      ...
      .each((idx, el) => {
        ...
        if (el.tagName === "SELECT") Select.GetInstance(el.id)?.enable();
        else if (el.role === "tinymce") TinyMCE.GetInstance(el.id)?.enable();
  ...
  submit = (stepCheck = false, stepDirection = "+") => {
    ...
    let fail = (returnData) => {
      ...
      if (returnData.status === 500)
        alert("Er is een fout gebeurd bij het indienen van het formulier!");
    };
    ...
    $.each(data, (input, validation) => {
      ...
      if (validation.feedback)
        $(this.element)
          .find(`[name='${input}']`)
          .parent()
          .append(`<p class='invalid-feedback'>${validation.feedback}</p>`);
    });
  };
  ...
  setField = (name, value) => {
    ...
    switch (field.role || field.type) {
      ...
      case "checkbox":
        Checkbox.GetInstance(field.dataset.id || field.name).setValue(value);
        break;
